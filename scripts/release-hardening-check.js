...
mustInclude(
  ...
  'preload must expose a bridged API.',
);
mustInclude('src/preload/preload.js', /Object\.freeze/, 'preload API should be frozen.');
mustInclude(
  'src/main/services/ipc-handlers.js',
  /get-release-source/,
  'get-release-source IPC handler must be registered.',
);
mustInclude(
  'src/main/services/ipc-handlers.js',
  /fetch-audio-quota/,
  'fetch-audio-quota IPC handler must be registered.',
);
mustInclude(
  'src/main/services/common.js',
  /redact/i,
  'common logging must keep redaction support.',
);
mustInclude(
  'src/main/services/auth.js',
  /timeout/i,
  'Roblox API requests should keep timeout handling.',
);
mustInclude(
  'src/main/services/transfer-handlers.js',
  /pipeline|finished|createWriteStream/s,
  'transfer handlers must stream downloads safely.',
);
mustNotInclude(
  'src/renderer-react/index.html',
  /Content-Security-Policy[^>]*unsafe-eval/i,
  'renderer CSP must not allow unsafe-eval.',
);

const pkg = JSON.parse(read('package.json'));
if (pkg.build?.win?.signAndEditExecutable !== true) {
  errors.push(
    'package.json: app win.signAndEditExecutable should remain true for release metadata/icon editing.',
  );
}
